  ...
  return (
    <div className="flex gap-2" aria-hidden="true">
      {Array.from({ length: total }).map((_, i) => (
        <div
          key={i}
          className="h-1.5 rounded-full transition-all duration-400 display-n"
          style={{
          ...
          <div className="flex flex-col gap-0">
            {steps.map((s, i) => (
              ...
              >
                ...
                <h3
                  className={`font-serif text-2xl font-semibold mb-4 leading-snug transition-colors duration-300 ${activeIndex === i ? "text-foreground" : "text-foreground/40"
                    }`}
                >
                  {s.title}
                </h3>
                <p
                  className={`text-[14px] leading-relaxed mb-6 transition-colors duration-300 ${activeIndex === i ? "text-muted-foreground" : "text-muted-foreground/40"
                    }`}
                >
                  {s.description}
                </p>
